...
for p in Pincode.objects.all():  
    ...
    if p.state_code in states: 
        if(District.objects.filter(name=p.province).exists()==False):
            ...
            d = District.objects.create(name=p.province,district_official=u,rainfall=0.0,altitude=20,temprature=0,population=0,water_source=0,humidity=0.0,age_frequency_vector=[0,0,0],density=0,slums_count=0,wind=0,victim_ids=['pt'])
            p.district = d
            u.is_staff =True
            u.save()
            gp.user_set.add(u)
